...
RESULT_FILE = "/scratch/project_2009199/register-vs-genre/genre/training/models/result.txt"
CACHE = "/scratch/project_2009199/register-vs-genre/genre/training/cache/"
...
def argparser():
    ...
    ap.add_argument('--labels', type=json.loads, default=None, metavar='LIST-LIKE',
...
def read_dataset(options):
    """ Read the data from train.parquet & dev.parquet files"""
    print("Reading data")
    ...
    #                                "test":"/scratch/project_2002026/amanda/genre_classification/hf-dataset-gen/upload/test.jsonl.gz",
     #                               "validation": "/scratch/project_2002026/amanda/genre_classification/hf-dataset-gen/upload/validation.jsonl.gz"
      #                              },
       #                    cache_dir=CACHE
        #                  )
    ...
    if options.split in dataset[list(dataset.keys())[0]].column_names:
        ...
        assert all([i in all_cats_200plus for i in options.labels]), "All or some of given labels do not have >200 instances in the data"
        labels = [all_cats_200plus[i] for i in options.labels]
        dataset = dataset.map(wrap_cat2label(labels))
...
def find_max_f1_key(dictionary):
    """ This function assumes multi_label_metrics() returns values such as 'f1_th03'"""
    f1_th_keys = [key for key in dictionary.keys() if key.startswith('f1_th')]
    max_key = max(f1_th_keys, key=lambda x: dictionary[x]) if f1_th_keys else None
    return float(str(max_key)[-2] + "." + str(max_key)[-1])
...
def multi_label_metrics(predictions, labels, threshold):
    ...
    y_th04 = np.zeros(probs.shape)
    y_th04[np.where(probs >= 0.4)] = 1
    y_th03 = np.zeros(probs.shape)
    y_th03[np.where(probs >= 0.3)] = 1
    y_th02 = np.zeros(probs.shape)
    y_th02[np.where(probs >= 0.2)] = 1
    y_th06 = np.zeros(probs.shape)
    y_th06[np.where(probs >= 0.6)] = 1
    ...
    metrics = {'f1': f1_micro_average, # user-chosen or optimized threshold
               'f1_th02': f1_score(y_true=y_true, y_pred=y_th02, average='micro'),
               'f1_th03': f1_score(y_true=y_true, y_pred=y_th03, average='micro'),
               'f1_th04': f1_score(y_true=y_true, y_pred=y_th04, average='micro'),
               'f1_th05': f1_score(y_true=y_true, y_pred=y_th05, average='micro'),
               'f1_th06': f1_score(y_true=y_true, y_pred=y_th06, average='micro'),
               'accuracy': accuracy}
    return metrics
...
def train_multilabel(dataset, tokenizer, options):
    ...
    probs = sigmoid(torch.Tensor(predictions))
    # find best threshold
    results = multi_label_metrics(predictions, true_labels, threshold)
    best_th = find_max_f1_key(results)
    
    pred_labels = np.zeros(probs.shape)
    pred_labels[np.where(probs >= best_th)] = 1
...
if __name__=="__main__":
    ...
    print(dataset)
    print(dataset["train"]['label'][0:10])
